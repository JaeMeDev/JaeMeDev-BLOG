...
HTTP 메서드를 알아보기 전 요구사항을 보고 생각나는 대로 API를 설계해보자.
...
아직 아무것도 모르는 상태에서 URL을 설계할 경우에는 이런 식으로 할 수 있을 것이다.
...
위의 URL 설계는 좋은 URL 설계일까? URL을 설계할 때 중요한 것은 리소스를 식별하는 것이다. 그럼 이 `리소스`란 무엇일까?
...
다음으로 `PUT`메서드를 알아보자. `PUT` 메서드는 리소스를 대체 할 때 사용한다. 하지만 이 대체라는 개념을 잘 알고 있어야 `POST`, `PATCH`와 헷갈리지 않을 수 있다.
`PUT`은 이렇게 동작한다.
- 리소스가 있으면 대체
- 리소스가 없으면 생성
- 쉽게 이야기해서 덮어버린다.
...
비슷한 메소드인 `PATCH`를 알아보자. `PATCH`는 쉽게 `PUT`의 단점을 해결해주는 것이다. `PUT`의 단점은
원래의 데이터가 
```json
{
  "username": "lee",
...
이렇게 요청하게 되면, age 필드가 삭제되게 된다. 이런 단점을 보안한 것이 바로 `PATCH`이다.

* `PATCH`는 아직 지원이 안되는 서버들이 있다. 이런 경우에는 `POST`를 사용하면 된다.

마지막으로 알아볼 메서드는 `DELETE`이다. `DELETE`는 말그대로 리소스를 제거할 때 사용된다.
...
이번에는 `HTTP 메서드`에 대해서 알아보았다. 다음에는 웹 개발자라면 어느정도 알고 있는
`HTTP 상태코드`에 대해서 다뤄볼 예정이다.
