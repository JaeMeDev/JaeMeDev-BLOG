...
과거 사용했던 `RFC2616`의 `HTTP 헤더`는 이렇게 생겼었다.
...
- `Content-Type`은 표현 데이터의 형식을 설명한다.
- 미디어 타입, 문자 인코딩
- 예)
...
- `Content-Encoding`은 표현 데이터 인코딩이다.
- 표현 데이터를 압축하기 위해 사용
...
- `Content-Language`는 표현 데이터의 자연 언어를 표현한다.
- 표현 데이터의 자연 언어를 표현
- 예)
  - ko
  - en
...
일반 정보는 이런 것들이 있으며 한번씩 확인해보면 된다.
