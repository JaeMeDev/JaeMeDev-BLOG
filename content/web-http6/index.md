...
과거 사용했던 `RFC2616`의 `HTTP 헤더`는 이렇게 생겼었습니다.
...
이랬던 `RFC2616`이 폐기되고 `RFC7230` ~ `RFC7235`가 등장하였습니다.
...
첫 번째로 알아볼 것은 `Content-Type`입니다. `Content-Type`은 표현 데이터의 형식을 설명합니다.
- 미디어 타입, 문자 인코딩
- 예)
...
두 번째로 알아볼 것은 `Content-Encoding`입니다. `Content-Encoding`은 표현 데이터 인코딩입니다.
- 표현 데이터를 압축하기 위해 사용
...
세 번째로 알아볼 것은 `Content-Language`입니다. `Content-Language`는 표현 데이터의 자연 언어를 표현합니다.
- 표현 데이터의 자연 언어를 표현
- 예)
  - ko
  - en
...
네 번째로 알아볼 것은 `Content-Length`입니다. `Content-Length`는 표현 데이터의 길이입니다.
- 바이트 단위
- Transfer-Encoding(전송 코딩)을 사용하면 Content-Length 를 사용하면 안된다.

## 콘텐츠 협상

`협상(콘텐츠 네고시에이션)`은 클라이언트가 선호하는 표현 요청입니다.
...
전송방식은 말그대로 전송 방식이며 크게 4가지가 있습니다.
...
일반 정보는 이런 것들이 있으며 한번씩 확인해보면 됩니다.
...
전반적인 HTTP 에 대해서 살펴보았습니다. 지금까지 HTTP 포스팅을 보신분들께 큰 도움이 됬으면 좋겠습니다. 
