...
`HTTP 상태코드`란 클라이언트가 보낸 요청의 처리 상태를 응답을 통해 알려주는 기능을 하는 것이다.
아마 웹 개발을 경험해본 사람이라면 한번쯤은 봤을 것이다. 간단하게 정리하면 상태코드가 나타내는 의미는 이러하다.
...
우리는 상태코드를 전부알기란 어렵다. 그렇기 때문에 만약 모르는 상태 코드가 나타나면?
...
여기서 `리다이렉션`이란 웹브라우저가 `3xx`응답의 결과에 Location 헤더가 있으면, Location 위치로 자동 이동하는 것(리다이렉트)를 의미한다.
...
이렇게 알아보았고 그러면 지금까지 알아본 (302,307,303)중 무엇을 사용해야 할까?
- 302 : GET으로 변할 수 있음.
- 307 : 메서드가 변하면 안됨.
- 303 : 메서드가 GET으로 변경
- 역사
...
이번 포스팅에서는 HTTP 상태코드에 관하여 알아보았다. 다음 포스팅에서는 HTTP 헤더에 관해 알아볼 예정이니
많은 관심 부탁드린다.
